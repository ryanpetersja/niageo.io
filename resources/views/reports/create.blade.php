                ...
                <form method="POST" action="{{ route('reports.store') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            @if($clients->isEmpty())
                                <p class="mt-1 text-xs text-amber-600">No clients with active repositories or SSH servers. Add repos or servers to a client first.</p>
                            @endif
                        </div>
                        <div>
                            <x-input-label for="title" value="Report Title" />
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title')" required placeholder="e.g., Q1 2024 Development Report" />
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="date_from" value="From Date" />
                            <x-text-input id="date_from" name="date_from" type="date" class="mt-1 block w-full" :value="old('date_from', now()->subMonth()->startOfMonth()->format('Y-m-d'))" required />
                            <x-input-error :messages="$errors->get('date_from')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="date_to" value="To Date" />
                            <x-text-input id="date_to" name="date_to" type="date" class="mt-1 block w-full" :value="old('date_to', now()->subMonth()->endOfMonth()->format('Y-m-d'))" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <x-text-input id="uptime_score" name="uptime_score" type="number" step="0.01" min="0" max="100" class="mt-1 block w-full" :value="old('uptime_score')" placeholder="e.g., 99.95" />
                    ...
                    <div class="flex justify-end gap-4">
                        <a href="{{ route('reports.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">Cancel</a>
                        <x-primary-button>Create Report</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
